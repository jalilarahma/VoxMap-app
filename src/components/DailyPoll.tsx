...
export default function DailyPoll({ lang, onComplete }: DailyPollProps) {
  ...
  const handleVote = async (index: number) => {
    ...
    try {
      ...
      // Persist the user's vote so other screens (e.g. the map's vote-orb
      // share popup) can pull it out without round-tripping the DB.
      try {
        const today = new Date().toISOString().slice(0, 10);
        window.localStorage?.setItem(
          ...
          JSON.stringify({
            ...
            question_text: question ? getQuestionText(question, "en") : "",
          })
        );
      } catch {}
    } catch (e) {
  ...
  // Share UI lives on the map's vote orbs now (see WorldMap's Sentiment mode).
    ...
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-vox-dark px-4">
        <div className="text-center space-y-6 w-full max-w-lg">
          ...
          {/* Share buttons live on the map's vote orbs now — no inline share row here. */}
